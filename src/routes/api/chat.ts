...
- If the user reports one or more performed sets and also needs that live exercise replaced, preserve the attempt first: call \`mark_exercise_done\` with the exact performed sets, wait for its result, and only then call \`adjust_program\` in a later tool step. Never run those two mutations in parallel. The replacement will remain separate while the attempted sets stay in immutable workout history.
...
- When the user rejects proposed substitutes and reveals new equipment, call \`substitute_exercise\` again with every confirmed limitation and availability. The previous shortlist is obsolete. Do not move to the next exercise while the unavailable movement remains unresolved in the live panel: preserve reported sets, persist a replacement, or close the session honestly as partial.
